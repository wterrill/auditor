...
void totalDataSync(BuildContext context) async {
  // check if logged in with a test account:
  ...
  bool notATest = true;
  if (username == null || (username.toLowerCase().contains("mxotest") && portNumber == "88")) {
    Function callBack = () {
      notATest = false;
    };
    await Dialogs.notATestCheck(context, callBack);
  }
  if (notATest) {
    try {
